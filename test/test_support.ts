...
var defaultLibName =
    ts.getDefaultLibFileName(compilerOptions); /* Not sure what this is */
var libSource = fs.readFileSync(ts.getDefaultLibFilePath(compilerOptions),
                                'utf-8'); /* ??? */
...
export function parseFiles(nameToContent: StringMap): ts.Program {
  ...
  var compilerHost: ts.CompilerHost = {
    getSourceFile : function(sourceName, languageVersion) {
      if (nameToContent.hasOwnProperty(sourceName)) {
        return ts.createSourceFile(sourceName, nameToContent[sourceName],
                                   compilerOptions.target, true);
      }
      if (sourceName === defaultLibName) {
        if (!libSourceFile) {
          // Cache to avoid excessive test times.
          libSourceFile = ts.createSourceFile(sourceName, libSource,
                                              compilerOptions.target, true);
        }
  ...
  var program: ts.Program =
      ts.createProgram(entryPoints, compilerOptions, compilerHost);
